...
(function () {
  ...
  var TABLICA = {
    ...
    /* Порог времени — не длительность хода, а граница: разрыв между
       событиями, отсечка, окно занятости. Кривой у него нет и быть не
       может — кривая описывает движение, а здесь ничего не движется.
       Первое употребление — скролл галереи M7 (09.09): отсечка инерции
       и окно, пока смена занята. */
    'порог времени': { organ: 'слайдер', ot0: 0, do: 2000, edinica: 'мс' },
    ...
    // Толщина — ось шрифта wght и толщина обводки. Слово из словаря величин:
    // «вес» не употребляем, вес — это то, чего у величины нет. Шаг 10:
    // переменные шрифты слушаются мельче, но глаз различает десятками.
    'толщина':  { organ: 'слайдер', ot0: 100, do: 900, minShag: 10, edinica: 'wght' },
    // Пресет — пачка значений под одним именем (палитра из пар «фон + текст»,
    // набор кривых). Орган в ядре был с самого начала, а типа объявление не
    // знало, и стенды с пресетом оставались на сырых defs (11.09).
    'пресет':   { organ: 'preset', edinica: '' },
    ...
    // Вывод: число, полученное из других ручек, а не накрученное. Свод велит
    // выводить каждое число (П2), но вывод жил в коде — панель показывала
    // слагаемые и прятала результат. Ключа в P у строки нет: не хранится.
    'вывод':    { organ: 'vyvod', edinica: '' },
  };
  ...
  var OPORY = {
    ...
    // Исходник: доля от собственного размера материала — кадра, картинки,
    // видео. 100 — пиксель в пиксель на физической плотности экрана; выше
    // — растяжение, ниже — показ мельче исходника. Первое употребление —
    // потолок роста оригинала в галерее M7 (07.09): растяжение мылит, и
    // потолок должен называться размером материала, а не долей экрана.
    'исходник': { ot0: 25, do: 400, edinica: '% исходника' },
    // Путь: доля хода по лучу, дуге, траектории — «к какому месту пути».
    // Первое употребление — гашение имени в каталоге M7 (09.09): фейд
    // привязан не к часам, а к месту на луче, и тогда кривая хода его не
    // сдвигает. Ноль отдан под «сразу», потолок — конец пути.
    'путь':    { ot0: 1, do: 100, edinica: '% пути' },
  ...
  function izObyavleniya(zakon, mestaObj) {
    ...
    function pripisat(s, imyaSekcii, gdeSekcii) {
      ...
      spisok.forEach(function (m) {
        if (vMeste[m].indexOf(klyuch) < 0) vMeste[m].push(klyuch);   // отражение не двоит место
      });
    }
    ...
    var gdeSekcii = null, gdeGruppy, priSekcii = null, inspektorKlyuch = null;
    var dvoetochie = [];   // подписи, в которых объект прокрался к величине
    ...
    (zakon || []).forEach(function (s, nomer) {
      ...
      /* Подгруппа: второй уровень внутри секции. Величиной не является,
         ...
         МЕСТО У ПОДГРУППЫ ЧИТАЕТСЯ (11.09). Прежде строка 'g' пропускалась
         целиком, и `gde` на ней терялся молча: автор писал привязку, читал её
         как истину, а место оставалось пустым — клик по нему открывал панель,
         в которой нечего крутить. Поймано претензией свода на «Фото галереи»
         в каталоге M7: группы к месту привязаны, а место пустое. Секция даёт
         место всем своим строкам, подгруппа — своим, строка — себе. */
      if (s[0] === 'g') {
        gdeGruppy = s[2] && s[2].gde !== undefined ? s[2].gde : undefined;
        return;
      }
      if (s[0] === 'h') {
        sekcii.push(tek); tek = { imya: s[1], tipy: {} };
        gdeGruppy = undefined;                 // подгруппа кончилась вместе с секцией
        gdeSekcii = s[2] && s[2].gde !== undefined ? s[2].gde : null;
        priSekcii = s[2] && s[2].pri !== undefined ? s[2].pri : null;
        return;
      }
      /* ОТРАЖЕНИЕ — НЕ ВТОРОЙ ЗАКОН (11.09). Одну величину можно показать в
         нескольких кластерах: панель это умеет и склеивает показы. Компилятор
         про отражения не знал и считал повторное объявление новым законом —
         а значит, требовал обязательств заново: «Ход веера» отражён в
         «Скролле» без кривой рядом, и свод годами просил кривую, которая уже
         стоит в «Веере». Обязательства держит ПЕРВОЕ объявление; отражение
         только приписывается к своему месту, чтобы указка его знала. */
      if (typeof s[1] === 'string' && s[1].indexOf(':') > 0) dvoetochie.push(s[1]);
      var otrazhenie = !!klyuchiZakona[s[0]];
      /* ОБЯЗАТЕЛЬСТВО СНИМАЕТСЯ ТОЛЬКО С НАЗВАННОЙ ПРИЧИНОЙ (11.09). Время
         требует кривой, потому что время ведёт движение. Но временем меряют и
         ПОРОГ — «отсечка инерции» не движется, ей нечем изгибаться, и свод
         годами просил у неё кривую. Опция `bez` снимает обязательство и
         обязана сказать, почему: снятие без причины — само по себе претензия.
         Причина уходит в подсказку ручки, чтобы жила на виду. */
      var bez = s[3] && s[3].bez;
      if (bez !== undefined && (typeof bez !== 'string' || !bez.trim())) {
        pretenzii.push('закон ' + s[0] + ' снимает обязательство типа «' + s[2] +
                       '», не назвав причины: `bez` — это строка с объяснением');
      }
      if (!otrazhenie && !bez) {
        tek.tipy[s[2]] = (tek.tipy[s[2]] || 0) + 1;
        if (s[3] && s[3].organ === 'datchik') tek.tipy['датчик'] = (tek.tipy['датчик'] || 0) + 1;
      } else if (!otrazhenie && bez) {
        /* Снятая ручка всё же в секции: тип считаем, чтобы счётчики и
           отчёты не врали, — но в свой ряд, который обязательств не просит. */
        tek.tipy['снято:' + s[2]] = (tek.tipy['снято:' + s[2]] || 0) + 1;
      }
      if (s[2] === 'этажи' && !inspektorKlyuch) inspektorKlyuch = s[0];
      pripisat(s, tek.imya, gdeGruppy !== undefined ? gdeGruppy : gdeSekcii);
      klyuchiZakona[s[0]] = 1;
    ...
    (zakon || []).forEach(function (s) {
      ...
      if (['color', 'ease', 'toggle', 'otkat', 'palitra', 'datchik', 'inspektor',
           'garnitura', 'gradient', 'skrab', 'pruzhina', 'vyvod', 'preset'].indexOf(baza.organ) >= 0) {
    ...
    /* ОБЪЕКТ В ПОДПИСИ — ЭТО ПОДГРУППА (11.09). Панель трёхуровневая: секция
       держит закон, подгруппа — объект, подпись — величину. Когда объект
       некуда деть, он просачивается в подпись через двоеточие: «Титул: кегль»,
       «Плашки текста: длина», «Курсор: инерция». Разведка по своду стендов
       нашла 76 таких строк на восьми стендах — это не разнобой слов, а
       подгруппа, записанная в подпись: поиск по ручкам ищет тогда не то, а
       три «длины» разных объектов читаются как три разные величины.
       Претензия СВОДНАЯ, одной строкой на стенд: семьюдесятью шестью строками
       она похоронила бы всё остальное. */
    if (dvoetochie.length) {
      pretenzii.push('объект в подписи через двоеточие у ' + dvoetochie.length +
        ' (' + dvoetochie.slice(0, 3).join(', ') +
        (dvoetochie.length > 3 ? ', …' : '') + '): объекту место в подгруппе, ' +
        'подписи — величина');
    }
